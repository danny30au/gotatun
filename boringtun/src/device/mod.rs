...
use crate::udp::{UdpTransport, UdpTransportFactory, UdpTransportFactoryParams};
use crate::x25519;
...
const BUFFER_POOL_ALLOC: usize = MAX_UDP_SIZE * 4;
...
pub struct DeviceHandle<UF: UdpTransportFactory> {
    device: Arc<RwLock<Device<UF>>>,
}
...
pub struct Device<UF: UdpTransportFactory> {
    ...
    port: u16,
    udp_factory: Box<UF>,
    connection: Option<Connection<UF>>,
...
pub(crate) struct Connection<UF: UdpTransportFactory> {
    udp4: Arc<UF::Transport>,
    udp6: Arc<UF::Transport>,
    listen_port: u16,
...
impl<UF: UdpTransportFactory> Connection<UF> {
    pub async fn set_up(device: Arc<RwLock<Device<UF>>>) -> Result<Self, Error> {
...
impl<UF: UdpTransportFactory> DeviceHandle<UF> {
    pub async fn new(
        udp_factory: UF,
        device: AsyncDevice,
        config: DeviceConfig,
    ) -> Result<DeviceHandle<UF>, Error> {
        Ok(DeviceHandle {
    ...
    async fn stop_inner(device: Arc<RwLock<Device<UF>>>) {
...
impl<UF: UdpTransportFactory> Drop for DeviceHandle<UF> {
    fn drop(&mut self) {
...
impl<UF: UdpTransportFactory> Device<UF> {
    ...
    pub async fn new(
        ...
        tun: tun::AsyncDevice,
        config: DeviceConfig,
    ) -> Result<Arc<RwLock<Device<UF>>>, Error> {
    ...
    async fn open_listen_socket(
        &mut self,
    ) -> Result<(Arc<UF::Transport>, Arc<UF::Transport>), Error> {
            ...
            while let Some((packet_owned, addr)) = dec_rx.recv().await {
                let parsed_packet = match rate_limiter.verify_packet(
                    ...
                    Err(TunnResult::WriteToNetwork(cookie)) => {
                        let packet_owned = buffer_pool.copy(cookie);
                        send_udp_tx
                    ...
                    .handle_verified_packet(parsed_packet, &mut dst_buf[..])
                {
                    ...
                    TunnResult::WriteToNetwork(packet) => {
                        ...
                        packet_owned.packet.clear();
                        if packet_owned.packet.try_reclaim(packet.len()) {
                            packet_owned.packet.extend_from_slice(packet);
                        } else {
                            buffer_pool.copy(packet);
                        }
                        send_udp_tx
                            .send((packet_owned, addr))
                            .await
        ...
        let receive_task = tokio::task::spawn(async move {
            ...
            loop {
                while packet_bufs.len() < 2 * max_number_of_packets {
                    packet_bufs.push(buffer_pool.take(4096));
                }
    ...
    async fn handle_outgoing(
        ...
        let receive_task = tokio::task::spawn(async move {
            let mut buffer_pool = BufferPool::new(BUFFER_POOL_ALLOC);
            loop {
                let src_buf = buffer_pool.borrow_mut(mtu);
                ...
                let packet_buf = buffer_pool.take(n);
        ...
        let encapsulate_task = tokio::task::spawn(async move {
            ...
            while let Some(packet_buf) = dec_rx.recv().await {
                ...
                match peer.tunnel.encapsulate(packet_buf.packet(), &mut dst_buf) {
                    ...
                    TunnResult::WriteToNetwork(packet) => {
                        let packet_buf = buffer_pool.copy(packet);
        ...
        let send_task_v6 = tokio::task::spawn(async move {
            while let Some((packet_buf, addr)) = packet_v6_rx.recv().await {
                let _ = udp6.send_to(packet_buf.packet(), addr.into()).await;
            }
        });
...
impl<UF: UdpTransportFactory> Drop for Device<UF> {
    fn drop(&mut self) {
